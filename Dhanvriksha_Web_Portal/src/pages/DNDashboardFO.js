...
export default function DNDashboardFO() {
  ...
  const getFdData = () => {
    let fddata = [];
    // axios.get
    getDataFromApi("fixedDeposit").then((res) => {
      console.log("fd DAta", res);
      res.data.map((status) => {
  ...
  const getRdData = () => {
    let rddata = [];
    // axios.get
    getDataFromApi(`recurringDeposit/getAllRD`).then((res) => {
      console.log("rd Data", res);
      res.data.map((status) => {
  ...
  const getLoanData = () => {
    let loandata = [];
    // axios.get
    getDataFromApi(`loan/getAllLoan`).then((res) => {
              ...
              <TabPanel value="1">
                {fdData.map((data, i) => (
                  <div
                    ...
                    onClick={() =>
                      navigate("/dashboard/KycApprovalForm", {
                        state: {
                          memberId: data.memberId,
                          applicationDate: data.applicationDate,
                          tenure: data.tenure,
                          amount: data.fdAmount,
                          interestRate: data.interestRa,
                        },
                      })
                    }
                    key={i}
